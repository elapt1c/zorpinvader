//! Discover the system's default network interface.
//!
//! On Linux this queries the kernel routing table via netlink to find the
//! interface associated with the default route (dst 0.0.0.0/0) that has the
...
use libc::{
    ...
    IF_NAMESIZE, NETLINK_ROUTE, NLM_F_DUMP, NLM_F_MULTI, NLM_F_REQUEST, NLMSG_DONE,
    NLMSG_ERROR, PF_NETLINK, SOCK_DGRAM,
};

/// Parsed route information (subset of what getroute needs).
#[derive(Debug)]
struct RouteInfo {
    dst_addr: Ipv4Addr,
    gateway: Ipv4Addr,
...
fn read_netlink(fd: RawFd, buf: &mut [u8], _seq: u32, _pid: u32) -> io::Result<usize> {
    ...
    loop {
        let n =
            unsafe { recv(fd, buf[total..].as_mut_ptr() as *mut c_void, buf.len() - total, 0) };
        if n < 0 {
            return Err(io::Error::last_os_error());
        }
...
fn parse_route(buf: &[u8], offset: usize, hdr: &NlMsgHdr) -> Option<RouteInfo> {
    ...
    if rtm.rtm_family != AF_INET as u8 || rtm.rtm_table != RT_TABLE_MAIN {
        return None;
    }
    ...
    while pos + mem::size_of::<RtAttr>() <= end {
        ...
        match attr.rta_type {
            ...
            RTA_GATEWAY => {
                if data_len >= 4 {
                    let mut o = [0u8; 4];
                    o.copy_from_slice(&buf[data_start..data_start + 4]);
                    info.gateway = Ipv4Addr::new(o[0], o[1], o[2], o[3]);
                }
            }
            RTA_DST => {
                if data_len >= 4 {
                    let mut o = [0u8; 4];
                    o.copy_from_slice(&buf[data_start..data_start + 4]);
                    info.dst_addr = Ipv4Addr::new(o[0], o[1], o[2], o[3]);
                }
            }
            RTA_PRIORITY => {
                if data_len >= 4 {
...
/// Discover the default network interface name.
///
/// Queries the kernel routing table and returns the interface associated with
/// the lowest-priority default route (destination 0.0.0.0/0) that has a
/// non-zero gateway.
///
/// Returns `Ok(None)` if no default interface could be determined.
pub fn get_default_interface() -> io::Result<Option<String>> {
    let fd = open_netlink()?;
    let seq = 0u32;
    let pid = unsafe { getpid() } as u32;
    ...
    while offset + mem::size_of::<NlMsgHdr>() <= total {
        ...
        if let Some(info) = parse_route(&buf, offset, hdr) {
            // Only consider default routes (dst == 0.0.0.0).
            if info.dst_addr == Ipv4Addr::UNSPECIFIED && !info.gateway.is_unspecified() {
                log::debug!(
                    "if: route: '{}' dst=0.0.0.0 gw={} priority={}",
                    info.if_name,
                    info.gateway,
                    info.priority
                );
                if info.priority < best_priority {
                    ...
                    best_name = Some(info.if_name.clone());
                }
            }
        }
